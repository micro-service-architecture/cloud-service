...
## 데이터 동기화 문제
`Order Service`를 2개 기동해보자. `Order Service`는 h2 DB 를 이용하여 독립적인 데이터베이스를 가지고 있을 수 있도록 구성해 놓았다. 그리고. 만약 `Users`의 요청을 분산 처리하기 위해 `Order Service`로 요청이 들어오면 Orders 데이터는 분산 처리되어 저장된다. 그렇다면 어떻게 해결해야 할까?
- 하나의 Database 사용
